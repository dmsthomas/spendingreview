# >>> file: app.py >>>
"""
UK Mock Spending Review – v2.0
Fixed tab layout so each tab’s content is displayed correctly. Restored summary and results.
"""
import re
import pathlib
...
st.title("💰 UK Mock Spending Review (v2.0)")
...
tax_cat = {
    grp: sum(changes_tax[r['name']] * r['delta_per_unit'] for r in rows)
    for grp, rows in tax_groups.items()
}
spend_cat = {
    grp: sum(changes_spend[r['name']] * r['baseline'] for r in rows)
            ...
            with st.expander(grp):
                for r in rows:
                    ...
                    prev = st.slider(
                        ...
                        value=changes_tax[r['name']], key=key,
                        label_visibility="collapsed"
                    )
                    newv = base + prev
                    st.markdown(
                        f"**{r['name']}**  "
                        f"<span style='color:grey'>{fmt_value(base,unit)}</span> → "
                        f"<span style='font-weight:700'>{fmt_value(newv,unit)}</span> {badge(prev*r['delta_per_unit'])}",
        ...
        for grp, rows in spend_groups.items():
            with st.expander(grp):
                for r in rows:
                    key = f"spend_{r['name']}"
                    base = r['baseline']
                    prev_pct = int(changes_spend[r['name']]*100)
                    newsp = base*(1+prev_pct/100)
                    prev = st.slider(
                        label=r['name'], min_value=int(r['min_pct']), max_value=int(r['max_pct']),
                        value=prev_pct, key=key, format="%d%%",
                        label_visibility="collapsed"
                    )
                    newsp = base*(1+prev/100)
                    st.markdown(
                        f"**{r['name']}**  "
                        f"<span style='color:grey'>£{base:,.0f} bn</span> → "
                        f"<span style='font-weight:700'>£{newsp:,.0f} bn</span> {badge(-(newsp-base))}",
                        unsafe_allow_html=True
                    )
    with c2:
        st.metric("Total receipts", f"£{total_receipts_new:,.0f} bn", f"{tax_delta:+.1f}")
...
with tab_results:
    ...
    with t1: st.subheader('Tax summary'); st.table(df_tax)
    with t2: st.subheader('Spend summary'); st.table(df_spend)
